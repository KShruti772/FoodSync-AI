...
> **Module Owner**: Shruti (`feature/ai-matching`)  
...
The FoodSync AI Matching Engine is a **transparent, deterministic heuristic ranking service**. Its sole responsibility is to evaluate available surplus food donations and rank verified nearby recipient organizations using verifiable mathematical criteria.

### Engineering Philosophy:
- **No Machine Learning / No LLMs**: The MVP does **not** use deep neural networks, embeddings, Large Language Models, vector databases, or external AI APIs.
...
- **Synchronous Execution**: The matching algorithm executes **synchronously** within the `POST /api/v1/donations` request handler, immediately writing ranked rows to the PostgreSQL `matches` table.
...
```mermaid
flowchart TD
    D["1. New Food Donation Posted\n(quantity_meals, food_type, expiry_time, lat, lng)"] --> CP["2. Query Candidate Pool\n(All active, verified users with role = RECIPIENT)"]
    
    CP --> EF{"3. Hard Eligibility Filtering\n• is_verified == true?\n• Distance <= effective_radius?\n• Dietary preference compatible?\n• Perishable storage capability?\n• Expiry time >= 30 mins remaining?"}
    
    EF -- Disqualified --> DQ["Excluded from Match Candidates"]
    EF -- Qualified --> CS["4. Compute Normalized Sub-Scores [0.0 - 1.0]\n• Distance Score (S_dist)\n• Expiry Urgency Score (S_urg)\n• Capacity Fit Score (S_cap)\n• Dietary Preference Score (S_pref)"]
    
    CS --> WS["5. Linear Weighted Aggregation\nTotal Score = w_dist*S_dist + w_urg*S_urg + w_cap*S_cap + w_pref*S_pref"]
    
    WS --> TB["6. Deterministic Ranking & Tie-Breaking\n(Sort: Score DESC -> Distance ASC -> Capacity DESC -> Expiry ASC -> Recipient ID ASC)"]
    
    TB --> DB["7. Save Matches to PostgreSQL\n(Insert into matches table with status = PROPOSED)"]
    
    DB --> NT["8. Emit In-App Alert to Top Candidate\n(Insert into notifications table for Rank #1)"]
```
...
$$S_{total} = w_{dist} \cdot S_{dist} + w_{urg} \cdot S_{urg} + w_{cap} \cdot S_{cap} + w_{pref} \cdot S_{pref}$$

### Final Default Heuristic Weights:
$$\sum w_i = 1.00 \quad \implies \quad w_{dist} = 0.35, \quad w_{urg} = 0.30, \quad w_{cap} = 0.20, \quad w_{pref} = 0.15$$
